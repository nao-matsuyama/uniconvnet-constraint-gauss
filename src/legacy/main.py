...
# ==========================================
# ==========================================
# 臨時のモデル構造確認用コード（大文字小文字・パス完全吸収版）
import sys

# 存在する可能性のあるフォルダをすべて検索ルートに入れます
for folder in ["models_A_P2", "models_A-P2", "models"]:
    sys.path.append(os.path.join(os.path.dirname(__file__), folder))

# フォルダ内にある Python ファイルから UniConvNet_A を力づくで見つけます
UniConvNet_A = None
for folder in ["models_A_P2", "models_A-P2", "models"]:
    dir_path = os.path.join(os.path.dirname(__file__), folder)
    if os.path.exists(dir_path):
        for f in os.listdir(dir_path):
            if f.lower() in ["uniconvnet.py", "models.py"]:
                mod_name = f.replace(".py", "")
                try:
                    # フォルダ名付き、または直接インポートを試みます
                    mod = importlib.import_module(f"{folder}.{mod_name}")
                    UniConvNet_A = getattr(mod, "UniConvNet_A")
                except Exception:
                    try:
                        ...
                        continue
            if UniConvNet_A is not None:
                break
    if UniConvNet_A is not None:
        break

if UniConvNet_A is None:
    print("\n[ERROR] どうしても UniConvNet_A クラスが見つかりませんでした。")
    import sys

    sys.exit(1)

model = UniConvNet_A()
print("\n=== [SUCCESS] UniConvNet_A の自動読み込みに完全成功しました！ ===")
print("=== モデルのお尻の構造（最後の20行分） ===")
model_str = str(model).split("\n")
for line in model_str[-20:]:
    print(line)
import sys

sys.exit(0)  # 構造を出力したら安全に終了
# ==========================================
...
def get_args_parser():
    parser = argparse.ArgumentParser(
        "coinvlight training and evaluation script for image classification",
        add_help=False,
    )
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(
        "coinvlight training and evaluation script", parents=[get_args_parser()]
    )
