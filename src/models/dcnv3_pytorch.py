# coding:utf-8
"""
DCNv3 の純 PyTorch 実装 (CUDA コンパイル不要)。

OpenGVLab/InternImage の ops_dcnv3 (modules/dcnv3.py の DCNv3_pytorch と
functions/dcnv3_func.py の dcnv3_core_pytorch) を、CUDA 拡張 (`import DCNv3`)
への依存を取り除いて 1 ファイルに統合したもの。

F.grid_sample ベースで動作し、本物の DCNv3 と数値的に同一の変形可能畳み込みを
行う。CUDA 版より遅いが GPU 上で動作し、微分可能。

キー名 (dw_conv.0 / offset / mask / input_proj / output_proj) は公式実装と
一致するため、uniconvnet_t_1k_224_ema.pth の dcn パラメータがそのまま転移される。
"""
...
# ──────────────────────────────────────────────
# dcnv3_core_pytorch (functions/dcnv3_func.py より)
# ──────────────────────────────────────────────
...
def dcnv3_core_pytorch(input, offset, mask, kernel_h, kernel_w,
    ...
    ref = _get_reference_points(
        input.shape, input.device, kernel_h, kernel_w,
...
# ──────────────────────────────────────────────
# DCNv3_pytorch モジュール (modules/dcnv3.py より)
# ──────────────────────────────────────────────
...
class CenterFeatureScaleModule(nn.Module):
    def forward(self, query, weight, bias):
        return F.linear(query, weight=weight, bias=bias).sigmoid()
...
class DCNv3_pytorch(nn.Module):
    def __init__(self, channels=64, kernel_size=3, dw_kernel_size=None,
        ...
        if not _is_power_of_2(_d_per_group):
            warnings.warn(
                "You'd better set channels in DCNv3 to make the dimension of each "
                "attention head a power of 2 which is more efficient.")
